...
const getWordInfo = async(word)=>{
    ...
    try {
        resultDiv.innerHTML = "fetchin data...."
   const response = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`)
    ...
    resultDiv.innerHTML =
     ...
     <p> <strong>Meaning:</strong>${definitions.definition === undefined ? "NOt Found" : definitions.definition}
     </p>
     ...
     `
     if(definitions.antonyms.lengh=== 0){
        resultDiv.innerHTML += `<span>Not Found</span>`;
     }
     else{
        for(let i =0 ; i<definitions.antonyms.lengh; i++){
